...
import { useRef, useState, useEffect, useCallback, Suspense, type DragEvent } from "react";
...
/**
 * Full-screen webcam capture modal with aspect ratio selector, 3x3 reticle grid,
 * shutter flash, and real-time focus tips.
 */
function WebcamModal({
  ...
  const [aspectRatio, setAspectRatio] = useState<"1:1" | "4:3" | "full">("1:1");
  ...
  return (
    ...
    >
      {/* Top bar */}
      <div className="flex items-center justify-between px-4 py-3 bg-black/80 backdrop-blur-md border-b border-white/10 z-20">
        <button
          type="button"
          onClick={onClose}
        ...
        <div className="flex items-center gap-2 bg-white/10 rounded-full px-3 py-1 text-xs font-semibold text-white">
          <span className="size-2 rounded-full bg-emerald-400 animate-pulse" />
          {ready ? "Camera Live" : camError ? "Camera Error" : "Initializing..."}
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={cn(
              ...
              showGrid ? "bg-primary text-primary-foreground" : "bg-white/10 text-white hover:bg-white/20",
            )}
            aria-label="Toggle grid reticle"
      ...
      <div className="relative flex-1 overflow-hidden bg-black flex items-center justify-center">
        ...
        {camError ? (
          <div className="flex flex-col items-center gap-4 px-8 text-center max-w-sm">
            ...
            <p className="text-white font-semibold text-sm">{camError}</p>
            <button
              type="button"
          ...
          <>
            ...
            {ready && (
              <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center">
                <div
                  className={cn(
                    "relative border-2 border-emerald-400/80 rounded-3xl shadow-[0_0_30px_rgba(16,185,129,0.3)] transition-all duration-300",
                    aspectRatio === "1:1" ? "w-[75%] aspect-square" : aspectRatio === "4:3" ? "w-[85%] aspect-[4/3]" : "w-[92%] h-[82%]",
                  )}
                  aria-hidden
                >
                  {/* 3x3 Framing Grid */}
                ...
                <div className="mt-4 flex items-center gap-2 text-white/90 text-xs font-semibold px-4 py-2 rounded-full bg-black/60 backdrop-blur-md border border-white/10">
                  <Sparkles className="size-3.5 text-emerald-400" />
                  Place leaf center inside frame · Distance 10–15 cm
                </div>
              </div>
            )}
            ...
            {!ready && !camError && (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
                <div className="size-12 rounded-full border-4 border-white/20 border-t-emerald-400 animate-spin" />
                <p className="text-white/70 text-sm font-medium">Connecting camera stream...</p>
              </div>
            )}
          </>
        )}
      ...
      {/* Aspect Ratio & Shutter Bar */}
      <div className="bg-black/90 backdrop-blur-md py-5 px-4 flex flex-col items-center gap-4 border-t border-white/10">
        <div className="flex items-center gap-2 bg-white/10 p-1 rounded-full text-xs">
          {(["1:1", "4:3", "full"] as const).map((ratio) => (
            <button
              key={ratio}
              type="button"
              onClick={() => setAspectRatio(ratio)}
              className={cn(
                "px-3 py-1 rounded-full font-bold uppercase transition-colors",
                aspectRatio === ratio ? "bg-emerald-500 text-black shadow-sm" : "text-white/70 hover:text-white",
              )}
            >
              {ratio}
            </button>
        ...
        >
          <span className="size-14 rounded-full bg-emerald-500 block" />
        </button>
      </div>
          ...
          <div className="px-5 py-5">
            {phase === "pick" ? (
              <div className="animate-rise space-y-5">
                ...
                <div
                  ...
                  onDrop={handleDrop}
                  className={cn(
                    "surface-lift flex flex-col items-center p-6 text-center border-2 border-dashed transition-all rounded-3xl",
                ...
                >
                  ...
                  <h2 className="mt-4 font-display text-lg font-bold">Capture or Drop Leaf Photo</h2>
                  <p className="mt-1 text-xs text-muted-foreground">
                    Drag & drop your crop leaf image here, or use the camera / file selector below.
                  </p>
                  <ul className="mt-3 space-y-1.5 text-left text-xs text-muted-foreground">
                    <li>• Hold camera steady in daylight or natural lighting.</li>
                    <li>• Frame a single leaf centered inside the guides.</li>
                    <li>• Supports JPEG, PNG, WEBP multi-color diseased foliage.</li>
                  </ul>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="flex min-h-13 items-center justify-center gap-2 rounded-2xl bg-card text-sm font-semibold shadow-soft hover:bg-card/80 transition-colors"
                  >
                    <Images className="size-5 text-primary" aria-hidden /> {t("diagnose.gallery")}
                  </button>
                  <button
                    type="button"
                    onClick={() => fileRef.current?.click()}
                    className="flex min-h-13 items-center justify-center gap-2 rounded-2xl bg-card text-sm font-semibold shadow-soft hover:bg-card/80 transition-colors"
                  >
                    <Upload className="size-5 text-primary" aria-hidden /> {t("diagnose.upload")}
                  </button>
                </div>
                ...
                <button
                  ...
                  className="min-h-12 w-full rounded-2xl border border-dashed border-border text-sm font-semibold text-muted-foreground hover:bg-card/50 transition-colors"
                >
            ...
            {phase === "preview" && image ? (
              <div className="animate-rise space-y-4">
                <div className="relative overflow-hidden rounded-3xl shadow-lift border border-white/10">
                  <img
                    src={image}
                    alt="Selected leaf"
                  ...
                  <button
                    ...
                    className="absolute right-3 top-3 flex size-10 items-center justify-center rounded-xl bg-card/90 shadow-soft hover:bg-card transition-colors"
                  >
                    <X className="size-5" aria-hidden />
                  </button>
                </div>

                {/* Pre-Diagnostic Image Health Card */}
                <div className="surface p-4 rounded-2xl space-y-3">
                  <div className="flex items-center justify-between border-b border-border/40 pb-2">
                    <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                      <ShieldCheck className="size-4 text-emerald-500" /> Pre-Diagnostic Image Health
                    </span>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center text-xs">
                    <div className="bg-card/60 p-2 rounded-xl">
                      <p className="text-[10px] text-muted-foreground font-medium">Clarity / Focus</p>
                      <p className="font-bold text-emerald-400 mt-0.5 flex items-center justify-center gap-1">
                        <Eye className="size-3" /> Sharp
                      </p>
                    </div>
                    <div className="bg-card/60 p-2 rounded-xl">
                      <p className="text-[10px] text-muted-foreground font-medium">Lighting</p>
                      <p className="font-bold text-emerald-400 mt-0.5 flex items-center justify-center gap-1">
                        <Sun className="size-3" /> Optimal
                      </p>
                    </div>
                    <div className="bg-card/60 p-2 rounded-xl">
                      <p className="text-[10px] text-muted-foreground font-medium">Leaf Coverage</p>
                      <p className="font-bold text-emerald-400 mt-0.5">Adequate</p>
                    </div>
                  </div>
                </div>

                <label className="surface block p-4 text-xs rounded-2xl">
                ...
                <button
                  type="button"
                  onClick={run}
                  className="min-h-14 w-full rounded-2xl bg-primary text-base font-semibold text-primary-foreground shadow-lift hover:opacity-90 transition-all active:scale-[0.98]"
                >
                  {t("diagnose.start")}
                </button>
                <button
                  type="button"
                  onClick={reset}
                  className="min-h-12 w-full rounded-2xl bg-secondary text-sm font-semibold text-secondary-foreground hover:bg-secondary/80 transition-colors"
                >
                  {t("diagnose.retake")}
                </button>
              </div>
            ...
            {phase === "quality-failed" && quality?.issue ? (
              <div className="animate-rise space-y-4">
                ...
                <div className="surface p-5 rounded-3xl">
