...
export default function CalendarScreen() {
  ...
  // Browser-native TTS to read out the crop schedule
  const speakTimeline = () => {
    if (typeof window === "undefined" || !window.speechSynthesis) return;

    window.speechSynthesis.cancel();

    let text = "Here is your tomato crop timeline. ";
    if (plan) {
      text += `Your crop was sown on ${formatDate(plan.sowingDate)}. `;
      const current = plan.windows.find((w) => w.status === "current");
      if (current) {
        text += `Currently, you are in the ${current.stage.label} stage. Note: ${current.stage.farmerNote}. `;
      }
      text += `Expected harvest is from ${formatDate(plan.harvestStart)} to ${formatDate(plan.harvestEnd)}. ${plan.crop.harvestNote}`;
    }
            ...
            <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-[11px]">
              {plan.windows.map((w) => (
                <span key={w.stage.key} className="flex items-center gap-1.5">
                  <span
                    className={cn("size-2 rounded-full", STAGE_DOT[w.stage.tone])}
                    aria-hidden
                  />
                  {w.stage.label}
                </span>
